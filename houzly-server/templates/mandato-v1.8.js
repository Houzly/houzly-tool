/**
 * Template contratto di mandato Houzly — versione 1.8
 *
 * REGOLA: questo file non si modifica mai dopo che una pratica lo ha usato.
 * Per cambiare il testo si crea mandato-v1.9.js e si aggiorna templates/index.js.
 ...
 *                  categoria, cin
 *   commissione            → es. "30% (trenta per cento)" — è la commissione
 *                            TOTALE concordata, comprensiva degli eventuali
 *                            servizi accessori dell'art. 11-bis
 *   servizi11bisOggetto    → generato: "delle aree verdi pertinenziali e della
 *                            piscina" / "delle aree verdi pertinenziali" / "della piscina"
 *   sogliaSpesa            → es. "€ 50,00 (cinquanta/00)"
 *   preavvisoRecesso       → es. "6 (sei) mesi"
 *   foro                   → es. "Arezzo"
 *
 * Ogni comma può avere: { n, testo, elenco: [], elencoCondizionale: [], bold: true }
 * elencoCondizionale: [{ chiave, testo }] — la voce compare solo se
 * caso.condizioni[chiave] è true. Serve all'art. 11-bis.
 * Ogni articolo può avere: { id, numero, titolo, commi: [], soloSe: '<flag>' }
 * Premesse, articoli e commi con `soloSe` compaiono solo se il flag corrispondente
 * è vero. Flag calcolati dal generatore in base alla pratica:
 *   proprietaEsclusiva / proprietaCondivisa
 *   pagamentoUnico / pagamentoSuddiviso
 */

module.exports = {
  versione: '1.8',
  dataVersione: 'Settembre 2026 (rev. 4)',
  ...
  // Usato quando l'Immobile ha un solo proprietario.
  mandante:
  ...
  // Usato in comproprietà: intro, una voce per ciascun titolare, chiusura.
  mandanteMultiplo: {
    intro:
      'I sottoscritti, comproprietari dell\u2019Immobile nelle quote appresso indicate:',
    voce:
      '**{{nomeCognome}}**, nato/a a **{{luogoNascita}}** il **{{dataNascita}}**, residente / con ' +
      'sede in **{{residenza}}**, C.F. / P.IVA **{{cfPiva}}**, titolare della quota di ' +
      '**{{quota}}%**;',
    chiusura:
      'di seguito congiuntamente \u201cMandante\u201d o \u201cProprietari\u201d.',
  },

  premesse: [
    {
      lettera: 'a',
      soloSe: 'proprietaEsclusiva',
      testo:
    ...
    {
      lettera: 'a',
      soloSe: 'proprietaCondivisa',
      testo:
        'i Mandanti sono comproprietari dell\u2019immobile sito in **{{immobile.indirizzo}}**, piano ' +
        '{{immobile.piano}}, interno {{immobile.interno}}, identificato catastalmente al Foglio ' +
        '{{immobile.foglio}} Particella {{immobile.particella}} Subalterno {{immobile.subalterno}}, ' +
        'categoria catastale {{immobile.categoria}}, di seguito l\u2019\u201cImmobile\u201d, ciascuno ' +
        'per la quota indicata in epigrafe, come risultante dalla visura catastale allegata;',
    },
    {
      lettera: 'a-bis',
      soloSe: 'proprietaCondivisa',
      testo:
        'il sottoscrittore del presente contratto dichiara di agire in nome proprio e, quale ' +
        'mandatario con rappresentanza ai sensi degli artt. 1387 e ss. c.c., anche in nome e per ' +
        'conto degli altri comproprietari sopra indicati, dei quali dichiara di avere i poteri, ' +
        'manlevando la Mandataria da ogni conseguenza derivante dall\u2019eventuale difetto o ' +
        'eccesso di tali poteri;',
    },
    {
      lettera: 'b',
      testo:
  ...
  articoli: [
      ...
      commi: [
        {
          n: '5.1',
          testo:
            'Il Mandante consegna alla Mandataria, quale condizione per la sottoscrizione del ' +
            'presente contratto, **copia fronte e retro di un documento d\u2019identit\u00e0 in corso di ' +
            'validit\u00e0 del sottoscrittore e la visura catastale dell\u2019Immobile**, quest\u2019ultima ' +
            'necessaria alla verifica della titolarit\u00e0 e dei dati identificativi dichiarati.',
        },
        {
          n: '5.1-bis',
          testo:
            'Il Mandante consegna inoltre, non appena disponibili, la dichiarazione di conformit\u00e0 ' +
            'degli impianti, la planimetria catastale e ogni altro documento necessario o ' +
            'obbligatorio per lo svolgimento dell\u2019attivit\u00e0 ricettiva.',
        },
        {
          n: '5.2',
          testo:
    ...
    },
    {
      ...
      commi: [
        ...
        },
        {
          ...
          soloSe: 'proprietaEsclusiva',
          testo:
            'Il bonifico \u00e8 effettuato sull\u2019IBAN **{{mandante.iban}}**, intestato al Mandante. ' +
            'Ogni variazione deve essere comunicata per iscritto e ha effetto dal mese successivo ' +
            'alla comunicazione.',
        },
        {
          n: '9.4',
          soloSe: 'pagamentoUnico',
          testo:
            'I Mandanti concordemente indicano quale unico conto di accredito l\u2019IBAN ' +
            '**{{mandante.iban}}**, intestato a **{{mandante.nomeCognome}}**, sul quale la ' +
            'Mandataria verser\u00e0 l\u2019intero importo netto spettante. Con l\u2019accredito su tale conto ' +
            'la Mandataria \u00e8 liberata nei confronti di tutti i Mandanti, restando la ripartizione ' +
            'fra gli stessi rimessa ai loro rapporti interni. Ogni variazione deve essere ' +
            'comunicata per iscritto da tutti i Mandanti e ha effetto dal mese successivo.',
        },
        {
          n: '9.4',
          soloSe: 'pagamentoSuddiviso',
          testo:
            'I Mandanti richiedono che l\u2019importo netto spettante sia ripartito e accreditato a ' +
            'ciascuno in proporzione alla rispettiva quota di propriet\u00e0, sugli IBAN indicati in ' +
            'epigrafe. Ogni variazione deve essere comunicata per iscritto dall\u2019intestatario ' +
            'del conto interessato e ha effetto dal mese successivo alla comunicazione.',
        },
      ],
    },
    {
    ...
    },
    {
      id: 'art11bis',
      numero: 'Art. 11-bis',
      titolo: 'Servizi accessori di gestione {{servizi11bisOggetto}}',
      // Incluso solo se il Mandante ha attivato almeno uno fra verde e piscina.
      commi: [
        {
          n: '11-bis.1',
          testo:
            'Le parti danno atto che il mandato di cui all\u2019art. 2 comprende altres\u00ec la ' +
            'gestione ordinaria {{servizi11bisOggetto}}, nei termini seguenti:',
          elencoCondizionale: [
            {
              chiave: 'verde',
              testo:
                '**aree verdi pertinenziali**: cura del giardino, del prato, delle siepi e delle ' +
                'alberature, gestione e manutenzione degli impianti di irrigazione;',
            },
            {
              chiave: 'piscina',
              testo:
                '**piscina**: trattamento chimico delle acque, pulizia, manutenzione tecnica degli ' +
                'impianti, apertura e chiusura stagionale.',
            },
          ],
        },
        {
          n: '11-bis.2',
          testo:
            'La Mandataria delega l\u2019esecuzione materiale di tali servizi a professionisti terzi ' +
            'qualificati (a titolo esemplificativo: aziende agricole e di giardinaggio, ditte ' +
            'specializzate nel trattamento di piscine), da essa selezionati e coordinati. La ' +
            'Mandataria resta unico interlocutore operativo del Mandante per tali servizi, in ' +
            'coerenza con la logica del mandato.',
        },
        {
          n: '11-bis.3',
          testo:
            'La Mandataria organizza un sopralluogo tecnico con i fornitori incaricati, all\u2019esito ' +
            'del quale verranno determinati la consistenza e la frequenza degli interventi ' +
            'necessari, in relazione alle caratteristiche specifiche dell\u2019Immobile e delle sue ' +
            'pertinenze, nonch\u00e9 il corrispettivo economico dovuto ai fornitori per la prestazione. ' +
            'Tale corrispettivo resta a carico del Mandante e pu\u00f2 essere trattenuto ' +
            'sull\u2019accredito mensile secondo le modalit\u00e0 di cui all\u2019art. 11.5.',
        },
        {
          n: '11-bis.4',
          testo:
            'L\u2019attivit\u00e0 di organizzazione, coordinamento e supervisione svolta dalla Mandataria ' +
            'per i servizi di cui al presente articolo \u00e8 remunerata dalla **commissione di gestione ' +
            'di cui all\u2019art. 7**, che le parti danno atto essere stata concordata tenendo gi\u00e0 conto ' +
            'di tali servizi. **Nessuna commissione ulteriore, aggiuntiva o maggiorata \u00e8 dovuta alla ' +
            'Mandataria a questo titolo.**',
        },
        {
          n: '11-bis.5',
          testo:
    ...
    },
    {
      id: 'art11bis_escluso',
      numero: 'Art. 11-bis',
      titolo: 'Gestione del verde e della piscina',
      // Variante inclusa quando NON \u00e8 attivato alcun servizio accessorio:
      // senza di essa il contratto tacerebbe su chi se ne fa carico.
      commi: [
        {
          n: '11-bis.1',
          testo:
            'La gestione ordinaria delle aree verdi pertinenziali (giardino, prato, siepi, ' +
            'alberature, irrigazione) e della piscina (trattamento chimico delle acque, pulizia, ' +
            'manutenzione tecnica degli impianti, apertura e chiusura stagionale) non rientra nei ' +
            'servizi oggetto del presente mandato e non \u00e8 compresa nella commissione di cui ' +
            'all\u2019art. 7.',
        },
        {
          n: '11-bis.2',
          testo:
            'Ogni onere relativo resta in capo al Mandante, che vi provveder\u00e0 direttamente e a ' +
            'propria cura, garantendo comunque le condizioni di fruibilit\u00e0 e sicurezza ' +
            'dell\u2019Immobile per gli ospiti. Le parti potranno concordare per iscritto, in qualsiasi ' +
            'momento, l\u2019estensione del mandato a tali servizi.',
        },
      ],
    },
    {
  ...
  clausoleVessatorie: {
    ...
    elenco: [
      ...
      },
      {
        key: 'art11bis',
        label:
          'Art. 11-bis.5 \u2014 Limitazione di responsabilit\u00e0 della Mandataria per le prestazioni ' +
          'eseguite dai fornitori terzi',
      },
